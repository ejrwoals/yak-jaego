...
> 2. **사용자**: `python app.py` (보고서 생성 및 주문 계산)
...
- `processed_inventory.sqlite3`: 시계열 통계 데이터 (월평균, 이동평균, 런웨이 등)
...
- **📈 시계열 분석**: 시간에 따른 소모량 변화 추세 분석 및 3개월 이동평균 계산
...
- `processed_inventory.sqlite3`: 시계열 통계 데이터
  - 월평균_조제수량, 월별_조제수량_리스트, 3개월_이동평균_리스트, 런웨이 등
...
### app.py - 워크플로우 선택 및 실행 (사용자용)

#### 주요 특징:
- **DB 준비 상태 확인**: 두 개의 DB 존재 및 데이터 확인
- **워크플로우 선택**: 보고서 생성 또는 주문 계산 선택
...
2. **약품코드 기준 시계열 통합** (`merge_by_drug_code`)
   ...
   - **최신 재고 자동 채택**: 각 약품별로 가장 최근 월의 재고를 역순 탐색하여 선택
   - **음수 재고 지원**: 마이너스 재고도 정확히 반영
...
```python
# 각 약품별로 최신 월부터 역순으로 탐색
for month in reversed(months):  # 2025-09 → 2025-08 → ...
    if 약품이_해당_월에_존재:
        재고수량 = 해당_월의_재고  # 음수 포함
        break  # 가장 최근 재고 발견
```
...
2. **관리자/사용자 워크플로우 분리**
   ...
   - `app.py`: 보고서 생성 및 주문 계산 (사용자, 일상적 사용)
   - 깔끔한 UX
...
```bash
# 기존 DB 삭제
rm recent_inventory.sqlite3 processed_inventory.sqlite3
