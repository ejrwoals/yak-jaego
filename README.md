...
- `user_settings.sqlite3`: 사용자 설정 저장 (이동평균 개월 수, 런웨이 임계값 등)
...
1. **📊 재고 관리 보고서**
   ...
   - 🆕 **신규 약품 자동 보정**: 사용 기간이 선택된 이동평균 기간보다 짧은 약품은 실제 사용 기간으로 나눠서 정확한 평균 계산 (예: 3개월 이동평균 선택 시 1개월만 사용된 약품은 1로 나눔)
   - 단일 런웨이 지표로 진입장벽 낮춤
   - 초보자 및 일상적 사용에 최적화
   - N개월 이동평균 기준 내림차순 정렬
...
- **📂 데이터 파일 관리**: 데이터 관리 페이지(`/data/manage`)에서 월별 데이터 파일(XLS, XLSX, CSV)을 웹 UI로 관리
  - 드래그앤드롭 업로드, 파일 삭제, 파일 미리보기(10행), 파일 유효성 검증
  - 업로드 시 파일명에서 자동 추출된 년월 확인 모달 제공 (오인식 방지)
  - "아니요, 직접 선택" 옵션으로 사용자가 직접 년월 지정 가능 (원본 파일명 유지, DB에 월 매핑 저장)
  - 파일 목록에 "파일 수정 일시"와 "업로드 일시" 두 가지 날짜 표시
  - 연속적인 타임라인 시각화: 시작월~끝월 사이 모든 월을 표시하고, 누락 월은 회색 타일로 경고 표시
  - DB 재생성 버튼으로 새 데이터 즉시 반영
...
- **💡 약품 추천**: 약품 추천 페이지(`/patient/suggest`)에서 주기적 사용 패턴을 보이는 **전문약**에 대해 "이 약품을 사용하는 환자는 누구입니까?" 제안 제공
  ...
  - Pre-filtering: 전문약만 대상, 월평균 사용량 200개 이상인 약품 제외, 건너뛴 약품 제외 (gamification 효과)
...
- **⚙️ 사용자 설정 저장**: 자주 사용하는 설정값(이동평균 개월 수, 런웨이 임계값 등)을 DB에 저장하여 앱 재시작 후에도 유지
  - 우측 하단 톱니바퀴 버튼 클릭으로 설정 모달 열기
  - 드롭다운과 슬라이더로 설정값 조정
  - "기본값 복원" 버튼으로 초기값으로 되돌리기 가능

#### 워크플로우 1: 재고 관리 보고서
- **🎯 사용자 정의 이동평균**: 1~12개월 중 선택 가능 (기본 3개월)
- **🆕 신규 약품 자동 보정**: 사용 기간이 선택된 이동평균 기간보다 짧은 약품은 실제 사용 기간으로 나눠서 정확한 평균 계산
- **🏷️ 신규 약품 태그**: 테이블에서 신규 약품에 파란색 `신규` 태그 + `?` 아이콘 표시, 클릭 시 보정 방식 상세 정보 모달 제공
- **📊 단일 런웨이 지표**: 하나의 이동평균만 사용하여 직관적 분석
- **⬇️ 스마트 정렬**: N개월 이동평균 기준 내림차순 정렬 (많이 쓰는 약품 우선)
- **💊 전문약/일반약 분리 분석**: 조제수량(전문약) 또는 판매수량(일반약) 기준 선택 가능
- **📦 선택적 재고 업데이트**: 보고서 생성 전에 PMPLUS 약품별조제판매현황 파일(.xls)을 업로드하여 최신 재고 상태 반영 가능 (선택사항 - 파일 없이 기존 DB 기반으로도 생성 가능)
...
- **🎚️ 사용자 정의 강조 임계값**: 슬라이더로 런웨이 기준 조절 (0.1~3개월, 기본 1개월, 0.1개월 단위)
- **⚙️ 개별 임계값 설정**: 특정 약품에 별도 재고/런웨이 임계값 설정, 전용 모달에서 상태 카드로 관리
- **🚨 긴급 주문 식별**: 글로벌 임계값 미만 약품 자동 하이라이트 (메인 테이블)
- **🆕 신규 약품 알림**: 시계열 데이터가 없거나 사용 기간이 3개월 미만인 약품 자동 감지, 사이드바 책갈피로 표시 (hover 시 펼쳐짐)
...
1. **지원하는 파일 형식**:
   ...
   - **웹 UI에서 업로드 가능**: 데이터 관리 페이지(`/data/manage`)에서 드래그앤드롭으로 파일 업로드
...
3. **파일명 규칙**:
   - 지원 형식: `YYYY-MM.*`, `YYYYMM.*`, `YYYY_MM.*`, `YYYY년 MM월.*`, `YY-MM.*`, `YY_MM.*` (확장자: csv, xls, xlsx)
   - 2자리 연도 규칙: 00-49 → 2000-2049, 50-99 → 1950-1999
   - 예시: `2025-01.csv`, `202501.xlsx`, `2025_01.xls`, `2025년 1월.csv`, `25-10.xls`, `25_01.csv`
...
1. **보고서 유형 선택**:
   - 전문약 보고서 (조제수량 기준)
   - 일반약 보고서 (판매수량 기준)
2. **재고 데이터 업데이트 (선택사항)**: PMPLUS 약품별조제판매현황 파일(.xls/.xlsx/.csv) 업로드 시 `recent_inventory.sqlite3`를 최신화한 뒤 보고서 생성 (파일 없이 기존 DB 기반으로도 생성 가능)
3. **이동평균 기간 선택**: 1~12개월 중 선택 (기본 3개월)
4. `drug_timeseries.sqlite3`에서 데이터 로드
5. 선택한 약품유형만 필터링
6. HTML 보고서 생성 및 브라우저에서 자동 열기
...
- 🆕 **신규 약품 표시**: 사용 기간이 짧은 약품에 파란색 태그 + ? 아이콘으로 보정 정보 제공
...
- 🆕 **신규 약품 책갈피**: 시계열 데이터가 없거나 사용 기간이 3개월 미만인 신규 약품 자동 감지, 우측 사이드바 책갈피로 표시 (사용 기간, 평균 사용량, 예상 런웨이 정보 제공)
...
├── user_settings_db.py            # user_settings.sqlite3 관리 모듈 (사용자 설정)
...
├── user_settings.sqlite3          # 사용자 설정 DB (자동 생성)
└── README.md                      # 프로젝트 문서
```
...
#### 자동 종료 메커니즘 (Heartbeat):
- 온프레미스 데스크톱 앱이지만 브라우저를 GUI로 사용하는 구조이므로, 브라우저 종료 시 서버도 자동 종료되어야 함
- 클라이언트(`base.html`)가 **10초 간격**으로 `/api/heartbeat`에 POST 요청 전송
- 서버(`web_app.py`)가 **120초(2분)** 동안 heartbeat를 수신하지 못하면 자동 종료
- 백그라운드 탭 throttling(Chrome 등에서 최소 1분 간격)에도 2분 타임아웃으로 충분히 대응
...
- **데이터 파일 관리** (`/data/manage`): 월별 데이터 파일 업로드/삭제/미리보기/검증 기능
...
- 테이블 스키마:
  ...
  CREATE TABLE data_files (
      filename TEXT PRIMARY KEY,  -- 원본 파일명
      month TEXT NOT NULL,        -- 매핑된 월 (YYYY-MM 형식)
      uploaded_at TEXT            -- 업로드 일시
  )
  ```
- JSON 직렬화로 Python 리스트 저장
- numpy 타입 자동 변환
- **data_files 테이블**: 원본 파일명과 월 매핑 정보 관리 (self-healing 동기화 지원)
...
- **Pre-filtering**:
  - 전문약만 대상 (일반약 제외)
  - 월평균 사용량 200개 이상인 약품 제외 (정규분포로 근사 가능)
  - avg_interval == 1인 약품 제외 (매달 사용되는 약품은 개별 환자 등록 불필요)
  - 건너뛴 약품 제외 (gamification 효과: 건너뛰기 시 분모 감소)
- **유사 약품 표시**: 추천된 약품이 어떤 등록 약품들과 유사한지 K개 표시
...
patterns = [
    ...
    r'(\d{2})[-_](\d{2})',  # 25-01, 25_01 (2자리 연도)
    # 추가 패턴...
]
# 2자리 연도 규칙: 00-49 → 2000-2049, 50-99 → 1950-1999
```
...
런웨이 경고 기준은 웹 UI에서 슬라이더로 조절할 수 있습니다 (0.1~3개월, 기본 1개월, 0.1개월 단위).
...
1. **"DB가 없습니다" 오류**
   ```
   해결 방법 1: 에러 페이지에서 "DB 초기화" 버튼 클릭 (터미널 불필요)
   해결 방법 2: python init_db.py 실행
   ```
