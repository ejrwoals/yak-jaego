...
2. `data/` 폴더에서 월별 CSV 파일 자동 감지 및 로드
3. 약품코드 기준으로 데이터 자동 통합
4. 선택한 모드에 따라 데이터 필터링
   - 전문약: 전체 기간 동안 조제수량이 0인 품목 자동 제외
   - 일반약: 전체 기간 동안 판매수량이 0인 품목 자동 제외
5. 시계열 통계 계산 (월평균, 3개월 이동평균, 런웨이)
6. 처리된 데이터를 `processed_inventory_{mode}.csv`로 저장
7. 경량 HTML 보고서 생성 및 브라우저에서 자동 열기

**출력 파일:**
- `processed_inventory_dispense.csv`: 전문약 통합 시계열 데이터
- `processed_inventory_sale.csv`: 일반약 통합 시계열 데이터
...
**보고서 유형:**
- **전문약 보고서**: 조제수량 기준, 전체 기간 조제수량이 0인 품목 제외
- **일반약 보고서**: 판매수량 기준, 전체 기간 판매수량이 0인 품목 제외

1. **빠른 트렌드 확인**: 테이블의 "트렌드" 컬럼에서 각 약품의 소모량 변화를 스파크라인으로 즉시 확인
   - 검정 점선: 실제 월별 소모량
...
**보고서 내용:**
- 📈 재고 현황 요약 통계 (총 약품 수, 데이터 기간, 총 재고수량, 월평균 총 소모량)
