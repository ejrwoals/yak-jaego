# 약국 재고 관리 및 분석 시스템 (Jaego)

> **⚡ 빠른 시작**: `python app.py` 명령 하나로 CSV 파일에서 HTML 보고서까지 자동 생성!

## 🏥 프로젝트 개요

**Jaego**는 약국의 재고 관리와 사용량 분석을 위한 Python 기반 자동화 도구입니다. CSV 형태의 약품 사용량 데이터를 분석하여 재고 운영 효율성을 높이고, 시각적으로 이해하기 쉬운 HTML 보고서를 생성합니다.

### 📊 주요 기능

- **CSV 파일 전용 처리**: CSV 파일에 특화된 안정적이고 빠른 데이터 처리
- **다중 인코딩 지원**: UTF-8, CP949, EUC-KR 인코딩 자동 감지
- **재고 분석**: 월평균 조제수량 계산 및 재고 소진 예상 기간(런웨이) 산출
- **인터랙티브 시각화**: Plotly 기반의 동적 차트 생성
- **전문적인 HTML 보고서**: 완전한 재고 현황 대시보드 자동 생성
- **통합 워크플로우**: 하나의 명령어로 전체 분석 과정 완료

## 🚀 빠른 시작

### 필수 요구사항

- Python 3.7 이상
- pip (Python 패키지 관리자)

### 설치 방법

1. **저장소 클론**
   ```bash
   git clone https://github.com/yourusername/jaego.git
   cd jaego
   ```

2. **가상환경 설정 (권장)**
   ```bash
   python -m venv .venv
   source .venv/bin/activate  # Windows: .venv\Scripts\activate
   ```

3. **의존성 설치**
   ```bash
   pip install -r requirements.txt
   ```

### 데이터 준비

1. **Excel 파일을 CSV로 변환**:
   - Excel에서 약품 사용량 파일을 엽니다
   - `파일` → `다른 이름으로 저장` → `CSV(쉼표로 분리)(*.csv)` 선택
   - `data/` 폴더에 저장합니다

2. **CSV 파일 형식 요구사항**:
   - `약품명`: 의약품명
   - `제약회사`: 제조회사명
   - `약품코드`: 고유 식별 코드
   - `재고수량`: 현재 재고량
   - `조제수량`: 기간별 사용량

## 📖 사용 방법

### 🎯 통합 실행 (권장)

```bash
python app.py
```

**전체 워크플로우가 자동으로 진행됩니다:**
1. `data/` 폴더의 CSV 파일 목록 표시
2. 분석할 CSV 파일 선택
3. 데이터 기간(개월 수) 입력
4. 자동 데이터 처리 및 검증
5. 처리된 데이터를 CSV로 저장
6. HTML 보고서 생성 및 브라우저에서 자동 열기

### 📂 개별 모듈 실행

필요시 개별 모듈을 실행할 수도 있습니다:

```bash
# 1. CSV 데이터 처리만
python read_csv.py

# 2. 보고서 생성만 (processed_inventory.csv 필요)
python generate_report.py
```

### 📊 지원 기능

- **파일 형식**: CSV 파일 전용 (UTF-8, CP949, EUC-KR 인코딩 지원)
- **데이터 처리**: 자동 데이터 검증 및 런웨이 계산
- **보고서**: HTML 대시보드 자동 생성

### 📋 생성되는 보고서 내용

- 📈 재고 현황 요약 통계
- 📊 인터랙티브 차트 (재고 분포, TOP 약품 등)
- 🔍 상세 재고 테이블 (검색/정렬 기능)
- ⚠️ 주의사항 약품 목록 (재고 부족/과다)
- 📱 반응형 디자인 (모바일 최적화)

## 📁 프로젝트 구조

```
jaego/
├── data/                          # 입력 CSV 데이터 폴더
│   ├── 12개월간사용량(24925-250926).csv
│   ├── 6개월간사용량(250325-250926).csv
│   ├── 9개월간사용량(241224-250926).csv
│   └── 3개월간사용량(250625-250926).csv
├── app.py                         # 🎯 메인 워크플로우 (권장)
├── read_csv.py                    # CSV 데이터 읽기 및 처리 모듈
├── generate_report.py             # HTML 보고서 생성 모듈
├── requirements.txt               # Python 의존성 목록
├── processed_inventory.csv        # 처리된 데이터 (생성됨)
├── inventory_report_YYYYMMDD_HHMMSS.html  # 생성된 보고서
└── README.md                      # 프로젝트 문서
```

## 🔧 상세 기능

### app.py - 통합 워크플로우

#### 주요 특징:
- **원스톱 솔루션**: 하나의 명령어로 전체 분석 과정 완료
- **단계별 진행**: 6단계로 나누어진 명확한 워크플로우
- **사용자 친화적**: 각 단계마다 진행 상황 표시
- **오류 처리**: 문제 발생시 친절한 안내 메시지

### read_csv.py - CSV 데이터 처리 엔진

#### 주요 기능:
- **다중 인코딩 지원**: UTF-8, CP949, EUC-KR 순차 시도
- **안정적인 CSV 처리**: CSV 파일에 최적화된 읽기 성능
- **데이터 검증**: 필수 컬럼 존재 여부 확인 및 대체 컬럼 제안
- **런웨이 계산**: 현재 재고량 ÷ 월평균 사용량으로 소진 예상 기간 계산
- **오류 복구**: 파일 읽기 실패시 다른 파일 선택 옵션 제공

#### 런웨이 계산 로직:
```python
runway_months = 재고수량 / 월평균_조제수량

# 표시 형식:
# 1개월 이상: "X.XX개월"
# 1개월 미만: "X.XX일" (1개월 = 30.417일)
```

### generate_report.py - 시각화 및 보고서

#### 보고서 구성요소:

1. **대시보드 요약**
   - 총 약품 수
   - 재고 없는 약품 수
   - 과다 재고 약품 수 (6개월 이상)
   - 총 재고 가치

2. **인터랙티브 차트**
   - 재고 분포 히스토그램
   - TOP 20 약품별 재고량
   - 제약회사별 재고 분포
   - 런웨이 분포 분석

3. **상세 데이터 테이블**
   - 실시간 검색 기능
   - 컬럼별 정렬
   - 페이지네이션
   - 모바일 반응형

4. **경고 시스템**
   - 재고 부족 약품 (1개월 미만)
   - 과다 재고 약품 (6개월 초과)
   - 색상 코딩으로 위험도 표시

#### 기술 스택:
- **Backend**: Python, Pandas
- **Visualization**: Plotly.js
- **Frontend**: HTML5, CSS3, JavaScript
- **Styling**: 현대적인 그라디언트 디자인, 반응형 그리드

## 📊 샘플 데이터

프로젝트에 포함된 샘플 CSV 데이터:
- 3개월, 6개월, 9개월, 12개월 기간별 사용량 데이터
- CSV 형태로 제공 (UTF-8 인코딩)
- 실제 약국 운영 패턴을 반영한 테스트 데이터

## 🛠️ 고급 설정

### 컬럼 매핑 커스터마이징

`read_csv.py`의 `required_columns` 리스트를 수정하여 다른 컬럼명 지원:

```python
required_columns = ['약품명', '제약회사', '약품코드', '재고수량', '조제수량']
```

### 보고서 스타일 커스터마이징

`generate_report.py`의 CSS 섹션을 수정하여 색상, 폰트, 레이아웃 변경 가능

### 알림 임계값 설정

런웨이 경고 기준 변경:
```python
# 1개월 미만 -> 부족 경고
# 6개월 초과 -> 과다 경고
```

## 🔍 문제 해결

### 일반적인 오류 및 해결방법

1. **CSV 파일 읽기 오류**
   ```
   해결: Excel에서 'CSV(UTF-8)(*.csv)' 형식으로 다시 저장
   ```

2. **컬럼명 불일치**
   ```
   해결: 프로그램이 자동으로 유사한 컬럼명 제안
   ```

3. **인코딩 오류**
   ```
   해결: UTF-8, CP949, EUC-KR 자동 감지 및 변환
   ```

4. **데이터 형식 오류**
   ```
   해결: Excel에서 숫자 형식 확인 후 CSV로 재저장
   ```