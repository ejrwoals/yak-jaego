...
3. **📦 약 주문 수량 산출**
   ...
   - 📈 인라인 차트: 테이블 행 클릭 시 트렌드 차트 + 주문량 계산기 확장
...
- **⚙️ 개별 임계값 설정**: 특정 약품에 별도 재고/런웨이 임계값 설정, 전용 모달에서 상태 카드로 관리 (v3.12)
- **📝 메모 관리**: 전용 페이지에서 모든 약품 메모 조회/편집/삭제, 카드 기반 UI (v3.13)
...
- **📈 인라인 트렌드 차트**: 테이블 행 클릭 시 조제수량 추이, 3개월 이동평균, 현재 재고 수평선 표시
...
- 📈 **인라인 트렌드 차트**: 테이블 행 클릭 시 실제 조제수량, 3개월 이동평균, 현재 재고 수평선 표시
...
5. **메모 관리 페이지 신규**
   - 📝 **전용 관리 페이지**: 메인 페이지 헤더 "메모 관리" 버튼으로 접근
   - 🗂️ **카드 기반 UI**: 모든 메모를 카드 형태로 표시, 전체 내용 확인 가능
   - 🔍 **통합 검색**: 약품명, 약품코드, 메모 내용으로 검색
   - 📊 **추가 정보 표시**: 현재 재고, 개별 임계값 정보 함께 표시
   - ⚠️ **덮어쓰기 방지**: 기존 메모 있는 약품 선택 시 경고 배너 + 기존 내용 자동 채움
   - 🏷️ **메모 존재 표시**: 검색 결과에 "메모 있음" 배지 표시

6. **기술 구현**
   - `drug_memos_db.py`: 통합 메모 DB 관리 모듈 (CRUD + `get_all_memos_with_details`)
   ...
   - `templates/memo_manage.html`: 메모 관리 전용 페이지
   - API 엔드포인트: `/api/memos` (목록), `/api/memo/<drug_code>` (삭제), `/api/update_memo`, `/api/get_memo`
...
2. **주문 보고서 통합 (분리된 UX)**
   - 📋 **메인 테이블 독립**: 메인 테이블은 글로벌 임계값만으로 정렬/강조 (개별 임계값 무관)
   - 📑 **책갈피 모달**: 사이드바에서 개별 설정 약품 전용 모달 조회
   - 📊 **상태 카드 섹션**: 모달 상단에 urgent(빨강)/warning(주황)/safe(초록) 상태 카드 표시
   - 📈 **프로그레스 바**: 현재 재고 vs 임계값 비율 시각화 (%)
   - 📦 **주문 권장량**: 재고 임계값 기준 부족분 자동 계산 표시
   - 🔄 **접기/펼치기**: 안전 약품은 기본 접힘 상태, 클릭으로 펼치기
   - ⚠️ **설정/편집 불가**: 주문 보고서에서는 조회만 가능, 설정은 관리 페이지에서만

3. **관리 페이지 기능 (유일한 설정 장소)**
...
5. **기술 구현**
   ...
   - `drug_order_calculator.py`: 보고서 생성 시 개별 임계값 모달 (상태 카드 섹션) 반영

### v3.11 업데이트 (주문 워크플로우 런웨이 임계값 슬라이더)

1. **사용자 정의 강조 임계값 슬라이더 추가**
   - 🎚️ **싱글 핸들 슬라이더**: 드래그로 런웨이 강조 기준 조절 (0.5~6개월)
   - 📊 **실시간 미리보기**: 슬라이더 조작 시 설정값이 info-box에 즉시 반영
   - 📱 **터치 지원**: 모바일에서도 터치 드래그로 조작 가능
   - 🎯 **기본값 1개월**: 기존 동작과 동일하게 유지

2. **동적 임계값 적용**
   - 🔄 **OR 조건 유지**: 런웨이 OR 3-MA 런웨이 중 하나라도 임계값 미만이면 강조
   - 📋 **보고서 헤더 표시**: 생성된 보고서에 사용된 임계값 명시
   - 🔢 **긴급 약품 카운트**: 설정한 임계값 기준으로 긴급 약품 수 계산

3. **기술 구현**
   - `templates/workflow_order.html`: 슬라이더 UI (CSS + JavaScript)
   - `web_app.py`: `runway_threshold` 파라미터 수신 및 전달
   - `drug_order_calculator.py`: `generate_table_rows()`, `generate_order_report_html()` 함수에 동적 임계값 적용
...
1. **듀얼 핸들 슬라이더 UI 추가**
   - 🎚️ **직관적인 슬라이더**: 드래그로 부족/충분/과다 경계값 조정
   - 🟡🟢🔵 **시각적 구간 표시**: 노란색(부족), 초록색(충분), 파란색(과다) 구간이 실시간으로 변경
   - 📊 **실시간 미리보기**: 슬라이더 조작 시 경계값이 즉시 표시됨
   - 📱 **터치 지원**: 모바일에서도 터치 드래그로 조작 가능

2. **사용자 정의 경계값 지원**
   - 기본값: 부족(≤3개월), 충분(3~12개월), 과다(>12개월)
   - 최소값: 1개월, 최대값: 24개월
   - 두 핸들이 교차하지 않도록 자동 제한 (low < high)

3. **보고서 동적 반영**
   - 📋 **모달 제목**: 설정한 경계값이 제목에 표시 (예: "런웨이 5개월 이하")
   - 📝 **섹션 설명**: 각 섹션의 안내 문구에 경계값 동적 반영
   - 🔢 **약품 분류**: 설정한 경계값에 따라 약품이 부족/충분/과다로 분류됨

4. **기술 구현**
   - `workflow_simple.html`: 듀얼 핸들 슬라이더 UI (CSS + JavaScript)
   - `web_app.py`: `threshold_low`, `threshold_high` 파라미터 수신 및 전달
   - `generate_single_ma_report.py`: 경계값 파라미터를 받아 동적 분류 적용

### v3.8 업데이트 (재고 분류 체계 개선)

1. **'과다' 재고 그룹 신규 추가**
   - 🔵 **과다 재고**: 런웨이 12개월 초과 약품을 별도 그룹으로 분류
   - ⚠️ **유효기간 만료 위험 경고**: 1년 이상 재고 소진에 걸리는 약품 식별
   - 📊 **골디락스 존 정립**: '충분' 그룹이 런웨이 3~12개월로 명확해짐

2. **재고 분류 기준 변경**
   | 그룹 | 이전 기준 | 변경 후 기준 |
   |------|----------|-------------|
   | 🟡 부족 | 런웨이 ≤ 3개월 | 런웨이 ≤ 3개월 (동일) |
   | 🟢 충분 | 런웨이 > 3개월 | **3개월 < 런웨이 ≤ 12개월** |
   | 🔵 과다 | - | **런웨이 > 12개월** (신규) |

3. **음수 재고 경고 배너 추가**
   - ⚠️ **상단 경고 배너**: 음수 재고 약품이 있을 때만 표시
   - 📋 **음수 재고 모달**: 클릭 시 음수 재고 약품 목록 확인 가능
   - 🔍 **사용 중/미사용 구분**: 이동평균 기준으로 비고 표시

### v3.7 업데이트 (UI 통일성 및 사용자 경험 개선)

1. **모달 UI 통일**
   - 🎯 **일관된 테이블 구조**: 모든 탭(긴급/부족/충분/과다/악성재고)에서 동일한 8개 컬럼 사용
   - 👁 **숨김 처리 기능**: 확인 완료된 약품을 눈 아이콘으로 숨김/표시 토글, "숨김 약품" 탭에서 일괄 관리
   - 📈 **스파크라인 추가**: 모든 모달 테이블에 트렌드 스파크라인 표시
   - 📊 **토글 뷰**: 부족/충분/과다 탭에서 테이블 ↔ 막대 그래프 전환 가능

2. **랜딩 페이지 간소화**
   - 🧹 **깔끔한 랜딩**: 기본적으로 약품 목록 테이블 숨김 처리
   - 🔍 **스마트 검색**: 검색어 입력 시에만 일치하는 약품 행 표시
   - 📊 **핵심 정보 집중**: 재고 현황 분포 인디케이터 + 사이드바 책갈피로 빠른 탐색
   - 📈 **인라인 차트**: 테이블 행 클릭 시 해당 행 아래에 상세 트렌드 차트 확장 (모달 대신)

3. **공통 테이블 컬럼 (8개)**
   | 컬럼 | 설명 |
   |------|------|
   | 숨김 | 눈 아이콘 (숨김/표시 토글) + 메모 버튼 |
   | 약품명 | 약품 이름 (30자 제한) |
   | 약품코드 | 고유 코드 |
   | 제약회사 | 제조사 (12자 제한) |
   | 재고수량 | 현재 재고 |
...
4. **탭별 특화 기능**
   - 🔴 **긴급 탭**: 공통 8개 + "마지막 조제월" 컬럼 추가 (재고 0, 이동평균 > 0)
   - 🟡 **부족 탭**: 공통 8개 + [막대 그래프 보기] 버튼 (런웨이 ≤ 3개월)
   - 🟢 **충분 탭**: 공통 8개 + [막대 그래프 보기] 버튼 (3개월 < 런웨이 ≤ 12개월)
   - 🔵 **과다 탭**: 공통 8개 + [막대 그래프 보기] 버튼 (런웨이 > 12개월)
   - ⚪ **악성재고 탭**: 공통 8개 컬럼 (이동평균 = 0, 재고 > 0)
   - ⚠️ **음수재고**: 상단 경고 배너로 표시 (재고 < 0)
   - 👁 **숨김 약품 탭**: 숨김 처리된 모든 약품을 한 곳에서 관리, 다시 표시 가능
