...
3. **📈 고변동성 약품 보고서**
   ...
   - 메인 테이블: CV 내림차순 정렬, 스파크라인, 인라인 차트
   - 📌 **단발성 약품 필터링**: 가중 등장률 20% 미만 & 최근 2개월 내 등장 없는 약품을 별도 분류, 우측 사이드바 책갈피 + 모달로 관리
...
- **📈 인라인 차트**: 테이블 행 클릭 시 통계 카드 + 트렌드 차트 표시
- **📊 스파크라인**: 테이블에서 사용량 추이 간략 표시
- **📌 단발성 약품 필터링**: 가중 등장률 기반 분류 - 최근 달일수록 높은 가중치(0.1~1.0) 적용, 최근 2개월 내 등장 약품은 안전장치로 제외
- **📑 단발성 약품 모달**: 우측 사이드바 책갈피 클릭 시 단발성 약품 목록 모달 표시, 메인 테이블과 동일한 스파크라인/인라인 차트 제공
...
2. **CV 임계값 설정**: 슬라이더로 고/중 경계, 중/저 경계 조절
3. `processed_inventory.sqlite3`에서 시계열 데이터 로드
4. 각 약품별 CV 계산 및 변동성 그룹 분류
5. HTML 보고서 생성 및 브라우저에서 자동 열기
...
- 인라인 차트: 행 클릭 시 통계 카드 + 트렌드 차트 표시
- 📌 **단발성 약품 책갈피**: 가중 등장률 20% 미만 & 최근 2개월 내 등장 없는 약품을 우측 사이드바 책갈피로 표시, 클릭 시 별도 모달에서 관리
...
3. **단발성 약품 필터링**
   - 가중 등장률 계산: 최근 달에 높은 가중치 부여 (가장 오래된 달 = 0.1, 가장 최근 달 = 1.0)
   - 분류 기준: 가중 등장률 20% 미만 AND 최근 2개월 내 등장 없음
   - 최근 등장 약품은 안전장치로 필터링에서 제외
   - 우측 사이드바 책갈피로 단발성 약품 개수 표시
   - 모달에서 스파크라인, 인라인 차트 등 메인 테이블과 동일한 기능 제공
